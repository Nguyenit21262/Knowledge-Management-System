      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(360px,1fr))] gap-8">
        {mockDocuments.map((resource) => {
          ...
          return (
            <article
              key={resource.id}
              className="rounded-[22px] border border-slate-200 bg-white px-9 py-8 shadow-[0_4px_14px_rgba(15,23,42,0.08)]"
            >
              <Link to={`/documents/${resource.id}`} className="block">
                <div className="mb-6 flex items-start justify-between">
                  <div className="flex items-center gap-3 text-[var(--theme-blue)]">
                    <ResourceIcon className="h-6 w-6" strokeWidth={1.8} />
                    <span className="text-[1.05rem] font-normal">{resource.type}</span>
                  </div>

                  <span className="rounded-xl bg-slate-100 px-4 py-2 text-[0.95rem] font-normal text-[var(--theme-blue)]">
                    {resource.subject}
                  </span>
                </div>

                <h2 className="mb-4 text-[2rem] leading-tight font-medium tracking-tight text-slate-950">
                  {resource.title}
                </h2>
